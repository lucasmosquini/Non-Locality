...
def SDP_LHS(m, k, rho, rho_sep, eta, detp, medicoes):
    ...
    sigma = [pic.HermitianVariable(f'Sigma_lambda[{i}]', (2, 2)) for i in range(k**m)]
    rho_q = rho * q + (1 - q) * rho_sep
    rho_eta = eta * chi + (1 - eta) * (pic.partial_trace(rho_sep, subsystems=1, dimensions=(2, 2))) @ \
              (pic.partial_trace(chi, subsystems=0, dimensions=(2, 2)))
...
def SDP_LHS(m, k, rho, rho_sep, eta, detp, medicoes):
    ...
    Criando o SDP com rastreamento de tempo.
    
    global SDP_total_time
    ...
    SDP_total_time += time.time() - start_time
    return P, solution, q

r'''

def apply_svd_to_measurements(medicoes):
    '''
    Aplica SVD a cada submatriz 2x2 dentro de `medicoes`.
    '''
    reduced_medicoes = np.zeros_like(medicoes, dtype=complex)

    for i in range(medicoes.shape[0]):
        u, s, vh = np.linalg.svd(medicoes[i], full_matrices=False)
        reduced_medicoes[i] = np.dot(u, np.dot(np.diag(s), vh))
    
    return reduced_medicoes
